		...
		$.extend({
			PromptBox:{
				...
				create : function(op){
					...
					this.element = $("<div class='Prompt_floatBoxBg' id='fb"+ops.name+"'></div><div class='Prompt_floatBox' id='"+ops.name+"'><div class='pt_content'></div></div>");
					$("body").prepend(this.element);
					this.blank = $("#fb"+ops.name);						//遮罩层对象
					this.content = $("#"+ops.name+" .pt_content");				//弹出层内容对象
