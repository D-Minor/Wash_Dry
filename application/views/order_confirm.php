...
<body>
	...
	<div class="content xys-content header-fix">
		...
		<div class="panel shadow">
			...
			<div class="panel-content" id="payMethod">
				...
				<div class="buy-item">
					<span class="square-btn" id="wxpay">微信支付</span>
	...
	<script type="text/javascript">
		function confirmorder(){
			...
			//准备订单相关数据，通过post下单
			postdata.address = $.trim($('#address').text());
			postdata.contact = $.trim($('#contact').text());
			//订单通过页面直接计算总金额yc2015-6-25
			var tprice=$.trim($('#totalFee span').text());
			var tpricenum = tprice-0;
			postdata.totalprice=100*tpricenum;
			if ($('#payMethod .buy-item .chosen').text() == "微信支付")
				postdata.payment = 1;
			else
				postdata.payment = 0;
			$.ajax({
			...
			dataType:'json',
	   		...
	   		success: function(msg){//用户名检测结果
	   			if(msg.result="success"){
	   				$.Prompt("下单成功！请完成支付！", 1500); //时间单位为ms
	   				//alert(msg.billid);
	   				sessionStorage.removeItem('couponid');
	   				sessionStorage.removeItem('addressid');
	   				if ($('#payMethod .buy-item .chosen').text() == "微信支付") {
						setTimeout(function() {
		   					location.href = siteurl + "order/wxpay/?showwxpaytitle=1";
		   				}, 1500);
					}
	   			}
	   			else {
	   				$.Prompt("下单失败，请重新下单！", 1500);
	   			}		
	     	}
			});
		}
